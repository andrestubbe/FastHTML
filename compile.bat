...
echo ========================================
echo FastHTML Native Library Builder
echo ========================================

:: Configuration
set LIB_NAME=fasthtml
...
:: Compile C++ source
cl.exe /O2 /arch:AVX2 /W3 /MD /EHsc /LD ^
   /I "%JAVA_HOME%\include" ^
...
if %ERRORLEVEL% == 0 (
    ...
    copy /Y build\%LIB_NAME%.dll src\main\resources\native\%LIB_NAME%.dll >nul
    echo [COPIED] Copied to src\main\resources\native\%LIB_NAME%.dll
) else (
    echo.
